...
@interface UIViewController()<MTTMediaPlayerViewDelegate>
@end

@interface MTTMediaPlayerView()<MTTMediaControlDelegate>
...
@implementation MTTMediaPlayerView
...
- (MTTMediaControl*)mediaControl {
    if (!_mediaControl) {
        _mediaControl = [[MTTMediaControl alloc] initWithFrame:self.bounds];
...
-(void)installMovieNotificationObservers
{
    ...
    // 创建YWReachability
    MTTReachability *reachability = [MTTReachability reachabilityForInternetConnection];
...
- (void)networkStateChange
{
    ...
    // 1.检测网络状态
    MTTReachability *wifi = [MTTReachability reachabilityForLocalWiFi];
    
    // 2.检测手机是否能上网络
    MTTReachability *connect = [MTTReachability reachabilityForInternetConnection];
