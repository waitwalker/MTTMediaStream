...
@implementation MTTMP4Atom

@synthesize type = _type;
@synthesize length = _length;

+ (MTTMP4Atom *)atomAt:(int64_t)offset size:(int)length type:(OSType)fourcc inFile:(NSFileHandle *)handle {
    MTTMP4Atom *atom = [MTTMP4Atom alloc];
...
- (MTTMP4Atom *)nextChild {
    if (_nextChild <= (_length - 8)) {
        ...
        _nextChild += len;
        len -= cHeader;
        return [MTTMP4Atom atomAt:offset+_offset size:(int)len type:fourcc inFile:_file];
    }
    return nil;
}

- (MTTMP4Atom *)childOfType:(OSType)fourcc startAt:(int64_t)offset {
    [self setChildOffset:offset];
    MTTMP4Atom *child = nil;
    do {
