...
#include "MTTNALUnit.h"


// --- core NAL Unit implementation ------------------------------

MTTNALUnit::MTTNALUnit()
    : m_pStart(NULL),
    m_cBytes(0){
}

bool
MTTNALUnit::GetStartCode(const BYTE *& pBegin, const BYTE *& pStart, int& cRemain){
...
bool
MTTNALUnit::Parse(const BYTE *pBuffer, int cSpace, int LengthSize, bool bEnd){
...
void
ScalingList(int size, MTTNALUnit *pnalu){
    long lastScale = 8;
    long nextScale = 8;
...
// --- slice header --------------------
bool
LFSliceHeader::Parse(MTTNALUnit *pnalu){
    switch (pnalu->Type()) {
    case MTTNALUnit::NAL_IDR_Slice:
    case MTTNALUnit::NAL_Slice:
    case MTTNALUnit::NAL_PartitionA:
...
LFSEIMessage::LFSEIMessage(MTTNALUnit *pnalu){
    m_pnalu = pnalu;
