...
@interface MTTAVEncoder ()

{
    // initial writer, used to obtain SPS/PPS from header
    MTTVideoEncoder *_headerWriter;

    // main encoder/writer
    MTTVideoEncoder *_writer;
...
@implementation MTTAVEncoder

@synthesize bitspersecond = _bitspersecond;

+ (MTTAVEncoder *)encoderForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate {
    MTTAVEncoder *enc = [MTTAVEncoder alloc];
...
- (void)initForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate {
    ...
    _headerWriter = [MTTVideoEncoder encoderForPath:path Height:height andWidth:width bitrate:(int)self.bitrate];
    _times = [NSMutableArray arrayWithCapacity:10];

    // swap between 3 filenames
    _currentFile = 1;
    _writer = [MTTVideoEncoder encoderForPath:[self makeFilename] Height:height andWidth:width bitrate:(int)self.bitrate];
...
- (BOOL)parseParams:(NSString *)path {
    ...
    MTTMP4Atom *movie = [MTTMP4Atom atomAt:0 size:(int)s.st_size type:(OSType)('file') inFile:file];
    MTTMP4Atom *moov = [movie childOfType:(OSType)('moov') startAt:0];
    MTTMP4Atom *trak = nil;
    if (moov != nil) {
        for (;; ) {
            trak = [moov nextChild];
            ...
            if (trak.type == (OSType)('trak')) {
                MTTMP4Atom *tkhd = [trak childOfType:(OSType)('tkhd') startAt:0];
    ...
    MTTMP4Atom *stsd = nil;
    if (trak != nil) {
        MTTMP4Atom *media = [trak childOfType:(OSType)('mdia') startAt:0];
        if (media != nil) {
            MTTMP4Atom *minf = [media childOfType:(OSType)('minf') startAt:0];
            if (minf != nil) {
                MTTMP4Atom *stbl = [minf childOfType:(OSType)('stbl') startAt:0];
                if (stbl != nil) {
    ...
    if (stsd != nil) {
        MTTMP4Atom *avc1 = [stsd childOfType:(OSType)('avc1') startAt:8];
        if (avc1 != nil) {
            MTTMP4Atom *esd = [avc1 childOfType:(OSType)('avcC') startAt:78];
            if (esd != nil) {
    ...
    @synchronized(self){
        ...
        if (!_swapping) {
            ...
            if (st.st_size > OUTPUT_FILE_SWITCH_POINT || self.bitrateChanged) {
                ...
                _swapping = YES;
                MTTVideoEncoder *oldVideo = _writer;
                ...
                _writer = [MTTVideoEncoder encoderForPath:[self makeFilename] Height:_height andWidth:_width bitrate:(int)self.bitrate];
...
- (void)onNALU:(NSData *)nalu {
    ...
    if (_pendingNALU) {
        MTTNALUnit nal(pNal, (int)[nalu length]);
